...
def summary(device):
    ...
    summary = probe_inference(model, dummy_input, device)

    summary.print_summary(summary_info)


def forward_compare(device):
    model1 = MODEL(pretrained=True)
    model2 = MODEL(pretrained=True)

    dummy_input = imagenet_input()

    summary1 = probe_inference(model1, dummy_input, device)
    summary2 = probe_inference(model2, dummy_input, device)
...
def deterministic_backward_compare(device):
    dummy_input = imagenet_input()
    dummy_target = imagenet_target(dummy_input)
    loss_func = nn.CrossEntropyLoss()
...
if __name__ == '__main__':
    ...
    summary(device)
    print('\n\n\n')
    forward_compare(device)
    print('\n\n\n')
    backward_compare(device)
    print('\n\n\n')
    deterministic_backward_compare(device)
